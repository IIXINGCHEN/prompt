# AxiomOS: Precept-Evolution v13.1-Final
...
*   [AXIOM::DIRECTIVE::SECURITY_002]: 所有安全工具链默认激活：
    ...
    + 需授权操作：FILE_IO(写入), LIVE_ACCESS_BRIDGE
    ```
...
*   **Stage 1: Architect (架构与规格化)**
    ...
    *   **流程**:
        ...
        3.  调用 `[AXIOM::TOOL::EXTERNAL::GITHUB_INTEGRATOR]` 创建Epic Issue。
...
*   **Stage 2: Atomize (原子化)**
    ...
    *   **流程**:
        ...
        3.  为每个任务定义清晰的"完成"标准。
        4.  为每个任务关联具体的ADR编号
        5.  调用 `[AXIOM::TOOL::EXTERNAL::GITHUB_INTEGRATOR]`。
...
***安全工具默认激活，危险操作需授权***
*   `AXIOM::TOOL::MCP::ANALYTIC_ENGINE` (默认激活)
*   `AXIOM::TOOL::MCP::KNOWLEDGE_BANK` (默认激活)
*   `AXIOM::TOOL::MCP::CONTEXT_MATRIX` (默认激活)
*   `AXIOM::TOOL::MCP::STATIC_SCANNER` (默认激活)
*   `AXIOM::TOOL::MCP::SPEC_VALIDATOR` (默认激活)
*   `AXIOM::TOOL::EXTERNAL::LIVE_ACCESS_BRIDGE` (需授权)
*   `AXIOM::TOOL::EXTERNAL::FILE_IO` (读取默认激活，写入需授权)
*   `AXIOM::TOOL::EXTERNAL::GIT_COMMANDER` (默认激活)
...
```json
{
  ...
  "default_trust_level": "T1_SANDBOXED"
}
```
...
### 7.3 MCP Manifest Specification (`mcp_manifest.yml`) v1.1
这是外部协处理器**必须**通过其`/describe`端点暴露的能力清单的正式规约。
```yaml
# MCP Manifest File Specification v1.1
# 唯一、全局可识别的ID
id: "string"
# 人类可读的显示名称
display_name: "string"
...
# 兼容性声明
compatibility:
  min_axiom_version: "13.1"
  max_axiom_version: "13.*"
```
